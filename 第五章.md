...
![使用单一编码矢量和线性层的权重矩阵的矩阵乘法的示例。 因为单动向量全部为0且单个为1，所以单个1的放置将充当矩阵乘法中的选择器。 使用权重矩阵和合成矢量中的阴影图案可视地显示这一点。 尽管是功能性的，但这种查找方法在计算上是昂贵且低效的，因为单向量向量乘以权重矩阵中的每个数并计算每行的总和。](./images/5-1.png)

使用单一编码矢量和线性层的权重矩阵的矩阵乘法的示例。 因为单动向量全部为0且单个为1，所以单个1的放置将充当矩阵乘法中的选择器。 使用权重矩阵和合成矢量中的阴影图案可视地显示这一点。 尽管是功能性的，但这种查找方法在计算上是昂贵且低效的，因为单向量向量乘以权重矩阵中的每个数并计算每行的总和。
